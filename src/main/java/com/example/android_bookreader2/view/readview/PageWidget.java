...
public class PageWidget extends BaseReadView {
    ...
    protected void drawCurrentPageArea(Canvas canvas) {
        ...
        canvas.save();
        canvas.clipPath(mPath0, Region.Op.XOR);//异或集
    ...
    protected void drawNextPageAreaAndShadow(Canvas canvas) {
        ...
        try {
            canvas.clipPath(mPath0);
            canvas.clipPath(mPath1, Region.Op.INTERSECT);//取交集
        } catch (Exception e) {
        }
    ...
    @Override
    protected void setBitmaps(Bitmap bm1, Bitmap bm2) {
        mCurPageBitmap = bm1;
        mNextPageBitmap = bm2;
    }
