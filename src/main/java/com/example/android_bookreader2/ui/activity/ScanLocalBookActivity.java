...
import com.example.android_bookreader2.view.recyclerview.EasyRecyclerView;
...
/**
 * Created by tangpeng on 2018/1/22.
 * 扫描本地书籍
 */
...
public class ScanLocalBookActivity extends BaseActivity implements RecyclerArrayAdapter.OnItemClickListener{
    ...
    private void queryFiles() {
        ...
        // 查询后缀名为txt与pdf，并且不位于项目缓存中的文档,用like进行模糊查询，有两个通配符：%百分号代表任意个字符
        Cursor cursor = getContentResolver().query(
                ...
                MediaStore.Files.FileColumns.DATA + " not like ? and ("
                        ...
                        + MediaStore.Files.FileColumns.DATA + " like ? or "
                        + MediaStore.Files.FileColumns.DATA + " like ? or "
                        + MediaStore.Files.FileColumns.DATA + " like ? )",//设置条件
                new String[]{"%" + bookpath + "%",//selectionArgs，这个参数是要配合第三个参数使用的，
                        "%" + Constant.SUFFIX_TXT,// 如果你在第三个参数里面有？，那么你在selectionArgs写的数据就会替换掉？
                        "%" + Constant.SUFFIX_PDF,
